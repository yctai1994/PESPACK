...
pub fn stepM(
    ...
    for (mvec, svec, pvec, 0..) |*m_k, *s_k, *p_k, k| {
        N_k = 0.0;
        for (self.resp, wvec) |resp_n, w_n| N_k += w_n * resp_n[k];

        p_k.* = N_k / wtot;

        tmp = 0.0;
        for (self.resp, wvec, xvec) |resp_n, w_n, x_n| tmp += resp_n[k] * w_n * x_n;

        m_k.* = tmp / N_k;

        tmp = 0.0;
        for (self.resp, wvec, xvec) |resp_n, w_n, x_n| tmp += resp_n[k] * w_n * abs2(x_n - m_k.*);
